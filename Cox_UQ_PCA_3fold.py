...
# Method: Apply Cox-PH model (with Ridge penalty) based on PCA transformed features(n=300), 3-fold 10-repeated CV
# Date: 12/13/2018
# Name: Donglei Yin
...
rkf = RepeatedKFold(n_splits=3, n_repeats=10, random_state=0) # 3-fold 10-repeated CV
...
print("Averaged c-index from 3-fold 10 repeated CV(training): {:.3f}".format(np.mean(c_index_train)))
print("Averaged c-index from 3-fold 10 repeated CV(test): {:.3f}".format(np.mean(c_index_test)))
